...
# init_rsize.sh
#
# modified version of init_resize.sh (from raspi-config deb package) to use in Kali Linux
# since Kali does not expand/fill the SD card like raspios does.
#
# Author:  Arul Selvan
# Version: Aug 30, 2020
#
# Steps:
#
# 1. mount the image with hdiutil (on mac) and copy this to /boot
# 2. copy any other stuff needed to /boot; touch /boot/ssh; touch /boot/avhai etc
# 3. Edit the /boot/cmdline.txt to include the following
#    init=/boot/init_resize.sh
# 4. unmount with diskutil (on mac) 
# 5. sudo dd bs=1m if=/path/to/kali.img of=/dev/rdisk2 [ensure rdisk2 IS your sdcard =]
# 6. stick the sdcard in Pi and boot, enjoy
#
...
sed -i 's| init=/boot/init_resize\.sh||' /boot/cmdline.txt
