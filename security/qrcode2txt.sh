...
#   Mar 20, 2025 --- Add header to the outputfile
################################################################################
...
my_source="https://github.com/aselvan/scripts/blob/master/security/qrcode2txt.sh"
...
add_header() {
  rm -rf $output_file
  echo "# Generated by: $my_name"  >  $output_file
  echo "# Source: $my_source"    >> $output_file
  echo "# Date:   `date +'%b %d, %Y %r'`" >> $output_file
  echo "" >> $output_file
}
...
while read -r file; do
  ...
  echo "" >> "$output_file"
  nofile=1
done < <(find "$dir_path" -name "*.${file_ext}" -type f)

if [ $nofile -eq 0 ] ; then
  log.error "There are no *.$file_ext files found under dir \"$dir_path\", check path & try again."
else
